...
const NotificationsCard = ({data, index, noticationState}) => {
    let [isRepling, setRepling] = useState(false)

    let {type,reply,createdAt ,comment,replied_on_comment ,user, user: {personal_info: {fullname,username, profile_img}}, blog:{ _id,blog_id, title}, _id:notification_id} = data
    ...
    return (
        <div className="p-6 border-b border-grey border-1-black"> 
...

            {
                isRepling ? 
                <div className="mt-8">
                    <NotificationCommentField _id = {_id} blog_author={user} index={index} replyingTo={comment._id} setReplying={setRepling} notification_id={notification_id} notificationData={noticationState} />
                </div>: ""
            }
