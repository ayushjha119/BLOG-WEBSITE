  ...
  User.findOne({ _id: req.user })
    .then((user) => {
      if (user.google_auth) {
        return res
          .status(403)
          .json({ error: "You can not change password of google account" });
      }
      bcrypt.compare(
        currentPassword,
            ...
            User.findOneAndUpdate(
              ...
              .catch((err) => {
                return res.status(500).json({
                  error:
                    "some error occured while saving new password, please try again",
                });
              });
          });
        }
    ...
    .then((comment) => {
      if (comment.parent) {
        Comment.findOneAndUpdate(
          ...
          .then((data) => console.log("deleted from parent"))
